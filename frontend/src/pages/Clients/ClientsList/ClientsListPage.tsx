...
export default function ClientsListPage() {
  ...
  const handleAdd = async (data: Omit<Client, 'id' | 'createdAt'>) => {
    try {
      await addClient({ ...data, managerId: data.managerId || currentUser?.id || '' })
  ...
  const columns: ColumnsType<Client> = [
    ...
    },
    {
      ...
      filters: CLIENT_STATUS_FILTER.slice(1).map((s) => ({ text: s.label, value: s.value })),
    ...
    },
    {
      ...
      sorter: (a, b) => (a.lastContactAt ?? '').localeCompare(b.lastContactAt ?? ''),
    },
    {
      title: 'Теги',
      dataIndex: 'tags',
