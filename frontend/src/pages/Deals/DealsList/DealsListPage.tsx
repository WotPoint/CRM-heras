...
const STATUS_FILTERS: { value: DealStatus | ''; label: string }[] = [
  { value: '', label: 'Все статусы' }, { value: 'new', label: 'Новая' },
  { value: 'negotiation', label: 'Переговоры' }, { value: 'proposal_sent', label: 'КП отправлено' },
  { value: 'awaiting_payment', label: 'Ожидает оплаты' }, { value: 'won', label: 'Выиграна' },
...
export default function DealsListPage() {
  ...
  }, [deals, clients, search, statusFilter, managerFilter, canViewManager])
  ...
  const handleAdd = async (data: Omit<Deal, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      await addDeal({ ...data, managerId: data.managerId || currentUser?.id || '' })
  ...
  const columns: ColumnsType<Deal> = [
    {
      title: 'Сделка', key: 'title',
      render: (_, d) => <Text strong style={{ color: '#1677ff', cursor: 'pointer' }} onClick={() => navigate(`/deals/${d.id}`)}>{d.title}</Text>,
    ...
    },
    {
      ...
      render: (_, d) => {
        ...
        return c ? <Text style={{ cursor: 'pointer', color: '#1677ff' }} onClick={(e) => { e.stopPropagation(); navigate(`/clients/${c.id}`) }}>{c.firstName} {c.lastName}</Text> : '—'
      },
    },
    { title: 'Статус', dataIndex: 'status', key: 'status', render: (s: DealStatus) => <DealStatusBadge status={s} /> },
    { title: 'Сумма', dataIndex: 'amount', key: 'amount', render: (a) => a ? `${a.toLocaleString('ru-RU')} ₽` : '—', sorter: (a, b) => (a.amount ?? 0) - (b.amount ?? 0), align: 'right' },
    { title: 'Менеджер', dataIndex: 'managerId', key: 'manager', render: (id) => <UserAvatar userId={id} showName /> },
    {
      title: 'Дедлайн', dataIndex: 'deadline', key: 'deadline',
      render: (d) => { if (!d) return <Text type="secondary">—</Text>; const isOver = dayjs(d).isBefore(dayjs()); return <Text style={{ color: isOver ? '#ff4d4f' : undefined, fontSize: 13 }}>{dayjs(d).format('D MMM YYYY')}</Text> },
      sorter: (a, b) => (a.deadline ?? '').localeCompare(b.deadline ?? ''),
    },
    {
      title: 'Обновлено', dataIndex: 'updatedAt', key: 'updated',
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => setAddOpen(true)}>Создать сделку</Button>
      </div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        {[
          ...
          <Card key={s.label} size="small" style={{ borderRadius: 10, flex: 1 }} styles={{ body: { padding: '10px 16px' } }}>
      ...
      <Card style={{ marginBottom: 16, borderRadius: 12 }} styles={{ body: { padding: '12px 16px' } }}>
        <Space wrap size={12}>
          <Input prefix={<SearchOutlined style={{ color: '#bbb' }} />} placeholder="Поиск по названию или клиенту..." value={search} onChange={(e) => setSearch(e.target.value)} allowClear style={{ width: 260 }} />
          ...
          <Segmented value={viewMode} onChange={(v) => setViewMode(v as ViewMode)}
            options={[{ value: 'board', icon: <ProjectOutlined />, label: 'Канбан' }, { value: 'table', icon: <TableOutlined />, label: 'Список' }]} />
        </Space>
      </Card>
      {viewMode === 'board' ? (
        ...
        <Card style={{ borderRadius: 12 }} styles={{ body: { padding: 0 } }}>
          <Table dataSource={filtered} columns={columns} rowKey="id" size="middle"
            ...
            summary={() => filtered.length > 0 ? (
              <Table.Summary fixed>
                <Table.Summary.Row>
                  <Table.Summary.Cell index={0} colSpan={3}><Text type="secondary" style={{ fontSize: 12 }}>Итого</Text></Table.Summary.Cell>
                  <Table.Summary.Cell index={3} align="right"><Text strong>{totalAmount.toLocaleString('ru-RU')} ₽</Text></Table.Summary.Cell>
      ...
      <AddDealModal open={addOpen} onClose={() => setAddOpen(false)} onSubmit={handleAdd} initialClientId={initialClientId} />
    </div>
  )
}
