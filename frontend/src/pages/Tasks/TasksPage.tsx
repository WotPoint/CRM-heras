...
export default function TasksPage() {
  ...
  const filtered = useMemo(() => {
    return tasks
      ...
      .sort((a, b) => {
        const ga = GROUP_ORDER.indexOf(getGroup(a)), gb = GROUP_ORDER.indexOf(getGroup(b))
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach((t) => { const g = getGroup(t); if (!groups[g]) groups[g] = []; groups[g]!.push(t) })
    return groups
  }, [filtered])

  const handleAdd = async (data: Omit<Task, 'id' | 'createdAt'>) => {
    try { await addTask(data); message.success('Задача создана') }
      ...
      <Card style={{ marginBottom: 16, borderRadius: 12 }} styles={{ body: { padding: '12px 16px' } }}>
        <Space wrap size={12} align="center">
          ...
          <Select value={statusFilter} onChange={setStatusFilter} style={{ width: 150 }}>
            <Option value="">Все статусы</Option><Option value="new">Новые</Option><Option value="in_progress">В работе</Option>
          </Select>
          <Select value={priorityFilter} onChange={setPriorityFilter} style={{ width: 140 }}>
            <Option value="">Все приоритеты</Option><Option value="high">Высокий</Option><Option value="medium">Средний</Option><Option value="low">Низкий</Option>
          </Select>
          {hasRole('supervisor', 'admin') && (
            <Select value={assigneeFilter || undefined} onChange={(v) => setAssigneeFilter(v ?? '')} style={{ width: 180 }} placeholder="Все исполнители" allowClear>
              {managers.map((m) => <Option key={m.id} value={m.id}>{m.name}</Option>)}
            </Select>
          )}
          <Segmented value={showDone ? 'all' : 'active'} onChange={(v) => setShowDone(v === 'all')}
            options={[{ value: 'active', label: 'Активные' }, { value: 'all', label: 'Все' }]} />
        </Space>
      </Card>
      {filtered.length === 0 ? (
        <EmptyState description="Задач не найдено" actionLabel="Создать задачу" onAction={() => setAddOpen(true)} />
      ) : (
            ...
            <div key={groupKey} style={{ marginBottom: 24 }}>
              ...
              <Space direction="vertical" style={{ width: '100%' }} size={6}>
                {groupTasks.map((task) => {
                  ...
                  return (
                    <Card key={task.id} size="small" style={{ borderRadius: 10, opacity: isDone ? 0.65 : 1, borderLeft: isOverdue ? '3px solid #ff4d4f' : isDone ? '3px solid #52c41a' : undefined }} styles={{ body: { padding: '10px 14px' } }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <Checkbox checked={isDone} onChange={() => toggleDone(task.id)} style={{ marginTop: 2, flexShrink: 0 }} />
                            ...
                            {task.deadline && (
                              <Tooltip title={dayjs(task.deadline).format('D MMMM YYYY, HH:mm')}>
                                <Text style={{ fontSize: 12, color: isOverdue ? '#ff4d4f' : '#999' }}>
                                  <CalendarOutlined style={{ marginRight: 3 }} />{dayjs(task.deadline).format('D MMM, HH:mm')}
                                </Text>
                              </Tooltip>
                            )}
                          </div>
