...
import {
  Card, Typography, Row, Col, Table, Select, Space,
  Progress, Tabs, Tag, Statistic, Divider,
} from 'antd'
import {
  BarChartOutlined, TeamOutlined, UserOutlined,
  CalendarOutlined, TrophyOutlined, RiseOutlined,
} from '@ant-design/icons'
import dayjs from 'dayjs'
import type { ColumnsType } from 'antd/es/table'

import { MOCK_DEALS, MOCK_CLIENTS, MOCK_ACTIVITIES, MOCK_USERS } from '@/mocks'
...
export default function ReportsPage() {
  ...
  const inPeriod = (date: string) =>
    dayjs(date).isAfter(periodStart) && dayjs(date).isBefore(periodEnd)

  // ── Данные для вкладки «Эффективность менеджеров» ───────
  const managerRows: ManagerRow[] = managers
    .filter((m) => !managerFilter || m.id === managerFilter)
    .map((m) => {
      const deals = MOCK_DEALS.filter((d) => d.managerId === m.id)
      const won = deals.filter((d) => d.status === 'won' && inPeriod(d.updatedAt))
      const lost = deals.filter((d) => d.status === 'lost' && inPeriod(d.updatedAt))
      const active = deals.filter((d) => !['won', 'lost'].includes(d.status))
      const closed = [...won, ...lost]
      const acts = MOCK_ACTIVITIES.filter((a) => a.managerId === m.id && inPeriod(a.date))
      return {
        id: m.id,
        name: m.name,
        clients: MOCK_CLIENTS.filter((c) => c.managerId === m.id).length,
  ...
  // ── Воронка ──────────────────────────────────────────────
  const funnelDeals = MOCK_DEALS.filter(
    (d) => !managerFilter || d.managerId === managerFilter
  )
  const funnelData = DEAL_COLUMNS.map((col) => {
    ...
    return {
      ...
      amount: colDeals.reduce((s, d) => s + (d.amount ?? 0), 0),
    }
  })
  ...
  // ── Клиенты по статусам ──────────────────────────────────
  const clientsData = [
    { label: 'Лиды',      value: MOCK_CLIENTS.filter((c) => c.status === 'lead').length,     color: '#1677ff' },
    { label: 'Активные',  value: MOCK_CLIENTS.filter((c) => c.status === 'active').length,   color: '#52c41a' },
    { label: 'Постоянные',value: MOCK_CLIENTS.filter((c) => c.status === 'regular').length,  color: '#722ed1' },
    { label: 'Архивные',  value: MOCK_CLIENTS.filter((c) => c.status === 'archived').length, color: '#d9d9d9' },
  ]
  const totalClients = clientsData.reduce((s, c) => s + c.value, 0)

  // ── Активности по типам ──────────────────────────────────
  const activitiesInPeriod = MOCK_ACTIVITIES.filter(
    (a) => inPeriod(a.date) && (!managerFilter || a.managerId === managerFilter)
  )
  const actByType = [
  ...
  const managerColumns: ColumnsType<ManagerRow> = [
    {
      title: 'Менеджер',
      key: 'name',
      render: (_, r) => <UserAvatar userId={r.id} showName showTooltip={false} />,
    },
    ...
    { title: 'Сделок', dataIndex: 'activeDeals', key: 'active', align: 'center' },
    {
      ...
      render: (_, r) => (
        <Space size={4}>
          <Tag color="green">{r.wonDeals} ✓</Tag>
          {r.lostDeals > 0 && <Tag color="red">{r.lostDeals} ✗</Tag>}
        </Space>
      ),
    },
    {
      ...
      key: 'amount',
      align: 'right',
      render: (v) => v ? <Text strong style={{ color: '#52c41a' }}>{v.toLocaleString('ru-RU')} ₽</Text> : '—',
      sorter: (a, b) => a.wonAmount - b.wonAmount,
    ...
    },
    {
      ...
      render: (v) => (
        <Space>
          <Progress
            percent={v} size="small" style={{ width: 64 }} showInfo={false}
            strokeColor={v >= 50 ? '#52c41a' : v >= 25 ? '#faad14' : '#ff4d4f'}
          />
    ...
    <div>
      ...
      <Tabs
        items={[
          {
            key: 'managers',
            label: <span><UserOutlined style={{ marginRight: 4 }} />Менеджеры</span>,
            children: (
          ...
          {
            key: 'funnel',
            label: <span><BarChartOutlined style={{ marginRight: 4 }} />Воронка</span>,
            children: (
                    ...
                    <div key={f.status}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        <Space>
                          ...
                          <Text strong style={{ fontSize: 14 }}>{f.label}</Text>
                        </Space>
                        <Space size={16}>
                          <Text type="secondary">{f.count} сделок</Text>
                          <Text strong>{f.amount.toLocaleString('ru-RU')} ₽</Text>
                        </Space>
                      </div>
                      <Progress
                        percent={Math.round((f.count / maxFunnel) * 100)}
                        showInfo={false}
                        strokeColor={f.color}
                        size={['100%', 14]}
          ...
          {
            key: 'clients',
            label: <span><TeamOutlined style={{ marginRight: 4 }} />Клиенты</span>,
            children: (
              <Row gutter={[16, 16]}>
                <Col xs={24} md={12}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                            ...
                            <Space>
                              ...
                              <Text type="secondary" style={{ fontSize: 12 }}>
                                ({totalClients > 0 ? Math.round((c.value / totalClients) * 100) : 0}%)
                              </Text>
                            </Space>
                          </div>
                          <Progress
                            percent={totalClients > 0 ? Math.round((c.value / totalClients) * 100) : 0}
                            showInfo={false}
                            strokeColor={c.color}
                            size="small"
                          />
                        ...
                        <Space direction="vertical" style={{ width: '100%' }} size={10}>
                          {entries.map(([src, cnt]) => (
                            <div key={src}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                <Text>{src}</Text>
                                <Text strong>{cnt}</Text>
                              </div>
                              <Progress
                                percent={Math.round((cnt / max) * 100)}
                                showInfo={false}
                                strokeColor="#667eea"
                                size="small"
                              />
          ...
          {
            key: 'activities',
            label: <span><CalendarOutlined style={{ marginRight: 4 }} />Активности</span>,
            children: (
                ...
                <Row gutter={[16, 16]}>
                  {actByType.map((a) => (
                    <Col key={a.label} xs={12} sm={6}>
                      <div style={{
                        ...
                        background: a.color + '10', borderRadius: 12,
                  ...
                  <Col span={24}>
                    ...
                    <Text type="secondary" style={{ fontSize: 13 }}>
                      Итого активностей за период:{' '}
                      <Text strong>{activitiesInPeriod.length}</Text>
                    </Text>
                  </Col>
                </Row>
              </Card>
