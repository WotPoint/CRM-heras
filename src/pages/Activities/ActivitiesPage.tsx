...
import {
  Card, Typography, Button, Select, Space, Tag, Tooltip,
  DatePicker, Divider, message,
} from 'antd'
import {
  PlusOutlined, PhoneOutlined, MailOutlined, TeamOutlined,
  FileTextOutlined, SwapOutlined, FilterOutlined,
} from '@ant-design/icons'
import { useNavigate, useSearchParams } from 'react-router-dom'
import dayjs from 'dayjs'

import { MOCK_ACTIVITIES, MOCK_USERS, MOCK_CLIENTS, MOCK_DEALS } from '@/mocks'
...
export default function ActivitiesPage() {
  ...
  return (
    <div>
      ...
      {grouped.length === 0 ? (
        <EmptyState
          description="Активностей не найдено"
          ...
          <div key={dayKey} style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <Text type="secondary" style={{ fontSize: 12 }}>
                {dayActivities.length} {dayActivities.length === 1 ? 'запись' : 'записей'}
              </Text>
            </div>
            ...
            <Space direction="vertical" style={{ width: '100%' }} size={8}>
              {dayActivities.map((activity) => {
                ...
                const client = activity.clientId
                  ? MOCK_CLIENTS.find((c) => c.id === activity.clientId)
                  : null
                const deal = activity.dealId
                  ? MOCK_DEALS.find((d) => d.id === activity.dealId)
                  : null
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 12, flex: 1 }}>
                        ...
                        <div style={{
                          ...
                          justifyContent: 'center', color: cfg.color, fontSize: 16,
                        }}>
                          {cfg.icon}
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                            ...
                            {deal && (
                              <>
                                {client && <Text type="secondary" style={{ fontSize: 12 }}>·</Text>}
                                <Text
                                  style={{ fontSize: 12, color: '#1677ff', cursor: 'pointer' }}
